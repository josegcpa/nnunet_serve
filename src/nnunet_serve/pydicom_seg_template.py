...
def _create_segment_dataset(data: dict) -> pydicom.Dataset:
    ...
    dataset.SegmentAlgorithmType = data.get(
    ...
    if "RecommendedDisplayCIELabValue" in data:
        dataset.RecommendedDisplayCIELabValue = data[
...
def rgb_to_cielab(rgb: List[int]) -> List[int]:
    ...
    tmp = tuple(
        ((x + 0.055) / 1.055) ** 2.4 if x > 0.04045 else x / 12.92 for x in tmp
    )
    ...
    tmp = tuple(
        x ** (1 / 3) if x > 0.008856 else (7.787 * x) + (16 / 116) for x in tmp
    )
    tmp = (
        (116 * tmp[1]) - 16,
        500 * (tmp[0] - tmp[1]),
